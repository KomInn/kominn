...
interface CommonFieldsState { usefulnessTypeChoices:Array<string> }
...
{    
    ...
    setUsefulnessType()
    {
        $.get(_spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('Forslag')/fields?$select=Choices&$filter=InternalName eq 'UsefulnessType'").then( (result:any) => { 
            
            if(result.d.results.length <= 0)
                console.error("Could not get usefulnesstype choices"); 
